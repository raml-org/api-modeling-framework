(ns api-modeling-framework.parser.domain.raml-types-shapes
                        ...
                        (mapv (fn [[k v]]
                                (let [property-name (utils/safe-str k)
                                      ...
                                      property-shape (if (nil? (utils/find-value property-shape (v/sh-ns "minCount")))
                                                       ;; mandatory prop?
                                                       (assoc property-shape (v/sh-ns "minCount") [(if required {"@value" 1} {"@value" 0})])
                                                       property-shape)
...
(defn parse-scalar [parsed-location scalar-type]
  (if (= scalar-type "number")
    {(v/sh-ns "or") {"@list" [(parse-scalar (utils/path-join parse-scalar "or/integer")
                                            "integer")
                              (parse-scalar (utils/path-join parse-scalar "or/float")
                                            "float")]}
